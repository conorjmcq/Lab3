...
mat4 ortho(float left, float right, float bottom, float top, float near_1, float far_1, mat4 orth_mat) {
	orth_mat = mat4(
					2 / (right - left), 0, 0, 0,
					0, 2 / (top - bottom), 0, 0,
					0, 0, -2 / (far_1 - near_1),0,
					-(left+right)/(right-left), -(top+bottom)/(top-bottom), -(far_1+near_1)/(far_1-near_1), 1
			   );
	return orth_mat;
}
void display(){
